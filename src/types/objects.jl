...
function create_object(properties=nothing)
...
#     # @napi_call create_object_dict(
#     #     pointer_from_objref(x)::Ptr{Cvoid}
#     # )::NapiValue
#     t = create_object()
#     t.get = (prop) -> @show prop  # get(x, prop, nothing)
#     t.set = (prop, value) -> x[prop] = value
#     t.has = (prop) -> haskey(x, prop)
#     t.keys = () -> keys(x)
